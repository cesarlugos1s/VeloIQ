...
import { projectsModelsGen } from './projectsSchema.gen';
import { projectsManualOverrides } from './projectsSchema.manual';
...
const baseNames = new Set(projectsModelsGen.map((m) => m.name));
const extraModels: ModelDef[] = projectsManualOverrides
...
export const projectsModels: ModelDef[] = [
    ...projectsModelsGen.map((m) =>
        mergeModel(m, projectsManualOverrides.find((o) => o.name === m.name))
    ),
    ...extraModels,
];
