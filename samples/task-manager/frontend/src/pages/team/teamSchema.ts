...
import { teamModelsGen } from './teamSchema.gen';
import { teamManualOverrides } from './teamSchema.manual';
...
const baseNames = new Set(teamModelsGen.map((m) => m.name));
const extraModels: ModelDef[] = teamManualOverrides
...
export const teamModels: ModelDef[] = [
    ...teamModelsGen.map((m) =>
        mergeModel(m, teamManualOverrides.find((o) => o.name === m.name))
    ),
    ...extraModels,
];
