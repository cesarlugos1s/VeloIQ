...
def _register_core_endpoints(app: FastAPI, engine, cfg: VeloIQConfig, *, extensions: list | None = None) -> None:
    ...
    async def licensing_status():
        ...
        """
        from veloiq_framework.licensing_utils import aggregate_license_pool
        return aggregate_license_pool(extensions)

    # ── Generate the Help-content-derived user guide PDF (admin only) ──────
    from fastapi import Depends as _Depends
    from veloiq_framework.auth.utils import require_role as _require_role

    @core_api.post("/help/generate-user-guide")
    async def generate_user_guide_endpoint(user: dict = _Depends(_require_role("Admin"))):
        from veloiq_framework.help.user_guide import generate_user_guide
        from veloiq_framework.cli.explorer import _find_project_root

        root = _find_project_root()
        if root is None:
            raise HTTPException(status_code=500, detail="Could not resolve host app root.")
        try:
            path = generate_user_guide(engine, extensions or [], root)
        except Exception as exc:
            raise HTTPException(status_code=500, detail=f"User guide generation failed: {exc}")
        return {"success": True, "path": str(path.relative_to(root))}
