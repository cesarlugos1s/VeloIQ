...
def _sync_extension_frontend(extensions: list, frontend_src: Path) -> None:
    ...
    lines.append(
        "export const globalDashboardTabHeaderComponents: "
...
def _ensure_app_tsx_header_button_wiring(frontend_src: Path) -> None:
    ...
    # Step C: Dashboard cell/tab suggestion wiring (Stage 4) -- a separate,
    # newer capability that must also reach hosts already fully wired for the
    # List/Show marker blocks above, so it runs unconditionally, independently
    # guarded by its own "already present" checks, same idiom as Step A.
    # Depends on _globalListHeaderButtons existing (either already present, or
    # just added by Step B above) -- if it's still missing (Step B's own
    # anchor wasn't found), the cellExtraActions half is silently skipped.
    if "globalDashboardTabHeaderComponents" not in content:
        import_re2 = re.compile(r'(import\s*\{[^}]*\})(\s*from\s*"\./extensions\.gen";)')
        m2 = import_re2.search(content)
        if m2:
            import_block = m2.group(1)
            patched_import = import_block[:-1].rstrip() + ", globalDashboardTabHeaderComponents }"
            content = content[:m2.start(1)] + patched_import + m2.group(2) + content[m2.end():]

    if _APP_TSX_DASHBOARD_TAB_MARKER not in content:
        dashboard_helper_block = (
            f"// {_APP_TSX_DASHBOARD_TAB_MARKER} — regenerated by `veloiq generate`, do not hand-edit\n"
            "const _globalDashboardTabHeaderActions = (tab: any, allModels: any[]) => (\n"
            "    <>{globalDashboardTabHeaderComponents.map((C, i) => createElement(C, { key: i, tab, allModels }))}</>\n"
            ");\n"
            "// VELOIQ:GLOBAL_DASHBOARD_TAB_HEADER:END\n\n"
        )
        anchor2 = re.search(r"const _renderList\b", content)
        if anchor2:
            content = content[:anchor2.start()] + dashboard_helper_block + content[anchor2.start():]

    def _add_dashboard_page_props(match: re.Match) -> str:
        inner = match.group(1)
        additions = ""
        if "cellExtraActions" not in inner and "_globalListHeaderButtons" in content:
            additions += " cellExtraActions={_globalListHeaderButtons}"
        if "tabExtraActions" not in inner and "_globalDashboardTabHeaderActions" in content:
            additions += " tabExtraActions={_globalDashboardTabHeaderActions}"
        if not additions:
            return match.group(0)
        return match.group(0)[:-2] + f"{additions} />"
