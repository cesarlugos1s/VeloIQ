"""safem db — Alembic migration wrappers."""
import os
import subprocess
import sys
...
def db():
    ...
    \b
    Common workflow:
      safem db upgrade          Apply all pending migrations
      safem db migrate -m "..."  Auto-generate a new migration
      safem db stamp             Mark current DB schema as up-to-date
      safem db downgrade -1      Roll back one migration
    """
...
def stamp(revision, env_file):
    ...
    \b
      safem db stamp        # marks current schema as 'head'
    """
...
def downgrade(revision, env_file):
    ...
    \b
    Examples:
      safem db downgrade -1       Roll back one step
      safem db downgrade base     Roll back everything
    """
