"""Permission primitives for SafeMantIQ: RoleDef, model_access, safem_field, rebac."""
...
# ── Default presets (used when SafemConfig.roles is not overridden) ───────────
...
def model_access(**role_actions: list[str]):
    ...
    """
    def _decorator(cls):
        cls.__safem_permissions__ = dict(role_actions)
...
def rebac(
    *,
    ...
    owner_field:
        Name of a column pointing to ``safem_user.id``.
        Shorthand for ``filter=lambda user, cls, _: cls.<field> == user["eid"]``.
    tenant_field:
        Name of a column pointing to ``safem_tenant.id``.
    ...
    if tenant_field is not None:
        ...
        def _tenant_fn(user, cls, session):
            ...
            from safemantiq_framework.auth.models import user_has_tenant_link
...
def safem_field(
    ...
    **kwargs: Any,
    ...
    if read_roles is not None:
        extra["safem_read_roles"] = read_roles
    if write_roles is not None:
        extra["safem_write_roles"] = write_roles
